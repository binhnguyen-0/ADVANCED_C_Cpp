...
- Tính đa hình (Polymorphism) có nghãi là nhiều dạng, và nó xảy ra khi có nhiều class cso liên quan với nhau thông qua kế thừa.
...
int main()
{
    ...
    dt = &sv1;                      // không in ra CN vì sv1 đã được ép kiểu ngầm định về class DoiTuong
    //dt->display();
    ((SinhVien*)dt)->display();     // down-casting: ép kiểu từ base class xuống derived class

    cout << endl;

    dt = &hs1;          // không in ra LH vì hs1 đã được ép kiểu ngầm định về class DoiTuong
    dt->display();
...
> ➡️ Kết quả:
>
> ![image](https://github.com/user-attachments/assets/83cf2850-80fc-4595-9d29-d2e915c47e46)
